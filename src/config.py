...
class ModelConfig:
    ...
    This class dynamically loads attributes from a JSON configuration file
    or keyword arguments. It mirrors the structure of the config source.

    Expected core attributes (from default.json or provided kwargs):
        model_type (str): Type of the model (e.g., 'transformer', 'base_model').
        hidden_dim (int): Hidden dimension size for model layers.
        output_dim (int): Output dimension size (e.g., number of classes).
        ...
        ff_dim (int): Feed-forward dimension in transformer layers.
        dropout (float): Dropout rate for regularization.
        attention_dropout (float): Dropout specific to attention layers.
        activation_function (str): Activation function to use (e.g., 'gelu', 'relu').
    ...
    Additional attributes can be present depending on the config file.
    """
    ...
    def _load_system_defaults(self):
        ...
        if os.path.exists(default_config_path):
            try:
                ...
                print(f"Warning: Could not load or parse configs/default.json: {e}. Using fallback defaults.")
        else:
            print(f"Warning: configs/default.json not found. Using fallback default values.")
        
        # Use loaded data or fallback if loading failed
        final_config_data = loaded_config_data if loaded_config_data is not None else _FALLBACK_DEFAULTS
    ...
    def from_json_file(cls, json_file: str) -> "ModelConfig":
        ...
        if not os.path.exists(json_file):
            print(f"Warning: Config file {json_file} not found. Initializing a default ModelConfig.")
        ...
        try:
            ...
            print(f"Warning: Could not load or parse {json_file}: {e}. Initializing a default ModelConfig.")
            return cls()
    ...
    def from_pretrained(cls, model_path_or_config_file: str) -> "ModelConfig":
        ...
        if os.path.isdir(model_path_or_config_file):
            potential_config_path = os.path.join(model_path_or_config_file, "config.json")
        ...
        elif os.path.isfile(model_path_or_config_file) and model_path_or_config_file.endswith(".json"):
            config_file_to_load = model_path_or_config_file
        
        if config_file_to_load:
            return cls.from_json_file(config_file_to_load)
        else:
            print(f"Warning: No specific config.json found for '{model_path_or_config_file}'. "
